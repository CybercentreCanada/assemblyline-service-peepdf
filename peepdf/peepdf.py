...
class PeePDF(ServiceBase):
    ...
    def extract(
        self, data: bytes, filename: str, request, description: str = "", *, supplementary: object = False
    ) -> None:
        ...
        with open(file_path, "wb") as f:
            f.write(data)
        if supplementary:
            request.add_supplementary(file_path, filename, description)
        else:
        ...
        if b"<pdf" in cbin and b"<document>" in cbin and b"<chunk>" in cbin:
            ...
            for chunk in chunks:
                if b"</chunk>" not in chunk:
                    leftover += chunk.replace(b"<document>", b"").replace(
                        b'<pdf xmlns="http://ns.adobe.com/xdp/pdf/">', b""
                    )
                    continue
                ...
                try:
                    ...
                    self.log.error(
                        "Found <pdf>, <document> and <chunk> tags inside an xdp file but could not "
    ...
    def parse_version_stats(self, stats_version: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if stats_version["Objects with JS code"]:
            v_json_body["objects_with_js_code"] = self.list_first_x(stats_version["Objects with JS code"][1])
    ...
    def analyze_javascript(
        self, js_code: str, unescaped_bytes: Collection[str], js_res: ResultSection, obj: int, request: ServiceRequest
    ) -> bool:
        ...
        if check_for_function("eval", js_code):
            ...
            eval_res.add_line(
                "This JavaScript block uses eval() function which is often used to launch deobfuscated JavaScript code."
            )
        if check_for_function("unescape", js_code):
            ...
            unescape_res.add_line(
                "This JavaScript block uses unescape() "
            ...
            if buff not in unescaped_bytes:
                ...
                if ";base64," in buff[:100] and "data:" in buff[:100]:
                    ...
                    try:
                        ...
                        self.log.error("Found 'data:;base64, ' buffer but failed to base64 decode.")
                        temp_path_name = None
        ...
        for i, buff in enumerate(set(unescaped_bytes)):
            ...
            shell_res = ResultSection(
                f"Unknown unescaped {len(unhexed)} bytes JavaScript "
    ...
    def peepdf_analysis(self, pdf_file: PDFFile, request: ServiceRequest) -> None:
        ...
        if (
            ", ".join(stats_dict["Errors"]) == "Bad PDF header, %%EOF not found, PDF sections not found, No "
        ...
        json_body = {
            ...
            "encryption_algorithms": [
                f"{algorithm_info[0]} {str(algorithm_info[1])} bits"
        ...
        res = ResultSection(
            "PDF File Information", body_format=BODY_FORMAT.KEY_VALUE, body=json.dumps(json_body), parent=request.result
        )
        ...
        for version, stats_version in enumerate(stats_dict["Versions"]):
            res_version = ResultSection(
                ...
                body=json.dumps(self.parse_version_stats(stats_version)),
            )
            ...
            for obj in stats_version["Objects"][1]:
                ...
                if cur_obj.containsJScode:
                    javascript_res.add_line(
                        f"Object [{obj} {version}] contains {len(cur_obj.JSCode)} block(s) of JavaScript"
                    )
        ...
        if js_dump:
            ...
            self.extract(temp_js_dump_bin, temp_js_dump, request, supplementary=True)
